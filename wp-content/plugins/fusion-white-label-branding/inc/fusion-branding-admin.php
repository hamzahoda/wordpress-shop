...
class Fusion_White_Label_Branding_Admin {
	...
	public function __construct() {
		...
		if ( $this->user_can_see_changes() ) {
			...
			add_action( 'add_meta_boxes', [ $this, 'update_builder_meta_box' ] );
			...
			add_filter( 'fusion_builder_admin_welcome_title', [ $this, 'fusion_builder_welcome_screen_title' ] );

			// Update Fusion Builder welcome screen about text.
			add_filter( 'fusion_builder_admin_welcome_text', [ $this, 'fusion_builder_welcome_screen_about_text' ] );

			// Update Fusion Builder welcome screen about text.
			add_filter( 'fusion_builder_admin_welcome_screen_content', [ $this, 'fusion_builder_welcome_screen_content' ] );
			...
			add_filter( 'fusion_fusionredux_args', [ $this, 'fusion_fusionredux_args' ] );
	...
	public function admin_menu() {
		...
		$welcome_tab   = add_menu_page( esc_attr__( 'Fusion White Label Branding', 'fusion-white-label-branding' ), esc_attr__( 'Fusion Branding', 'fusion-white-label-branding' ), $capability, 'fusion-white-label-branding-admin', [ $this, 'welcome_tab' ], 'dashicons-fusion-white-label-logo', '3.555555' );
		$support       = add_submenu_page( 'fusion-white-label-branding-admin', esc_attr__( 'Support / FAQ', 'fusion-white-label-branding' ), esc_attr__( 'Support / FAQ', 'fusion-white-label-branding' ), $capability, 'fusion-white-label-branding-support', [ $this, 'support_tab' ] );
		$settings      = add_submenu_page( 'fusion-white-label-branding-admin', esc_attr__( 'Branding Settings', 'fusion-white-label-branding' ), esc_attr__( 'Settings', 'fusion-white-label-branding' ), $capability, 'fusion-white-label-branding-settings', [ $this, 'branding_settings_tab' ] );
		$import_export = add_submenu_page( 'fusion-white-label-branding-admin', esc_attr__( 'Import / Export', 'fusion-white-label-branding' ), esc_attr__( 'Import / Export', 'fusion-white-label-branding' ), $capability, 'fusion-white-label-branding-import-export', [ $this, 'settings_import_export_tab' ] );

		add_action( 'admin_print_scripts-' . $welcome_tab, [ $this, 'admin_styles' ] );
		add_action( 'admin_print_scripts-' . $support, [ $this, 'admin_scripts_and_styles' ] );
		add_action( 'admin_print_scripts-' . $settings, [ $this, 'admin_scripts_and_styles' ] );
		add_action( 'admin_print_scripts-' . $import_export, [ $this, 'admin_scripts_and_styles' ] );
	}
	...
	public function admin_menu_remove_sub_menus() {
		...
		// Avada Options.
		$avada_options         = isset( $this->settings['fusion_branding']['avada'] ) ? $this->settings['fusion_branding']['avada'] : [];
		$avada_menus           = isset( $avada_options['remove_admin_menu'] ) ? $avada_options['remove_admin_menu'] : [];
		$avada_post_type_menus = isset( $avada_options['remove_post_type_menu'] ) ? $avada_options['remove_post_type_menu'] : [];

		// Fusion Builder Options.
		$fusion_builder_options = isset( $this->settings['fusion_branding']['fusion_builder'] ) ? $this->settings['fusion_branding']['fusion_builder'] : [];
		$fusion_builder_menus   = isset( $fusion_builder_options['remove_admin_menu'] ) ? $fusion_builder_options['remove_admin_menu'] : [];
		...
		if ( ! empty( $fusion_builder_menus ) ) {
			if ( 6 === count( $fusion_builder_menus ) ) {
				...
				foreach ( $fusion_builder_menus as $menu_slug ) {
					...
					if ( 'layouts' === $menu_slug ) {
						remove_submenu_page( 'fusion-builder-options', 'fusion-' . $menu_slug );
					} else {
	...
	public function fusion_branding_admin_styles() {
		...
		if ( ! empty( $avada_menus ) ) {
			foreach ( $avada_menus as $menu_slug ) {
				...
				if ( 'fusion-patcher' === $menu_slug ) {
					$style .= '#adminmenu .toplevel_page_avada .update-plugins { display: none; }';
				}
			}
		}
	...
	public static function header() {
		...
		<div class="fusion-white-label-branding-logo">
			<span class="fusion-white-label-branding-version">
				...
				printf( esc_html__( 'Version %s', 'fusion-white-label-branding' ), FUSION_WHITE_LABEL_BRANDING_VERSION ); // WPCS: XSS ok.
				?>
			</span>
		</div>
		<h2 class="nav-tab-wrapper">
			<?php
			self::admin_tab( esc_html__( 'Welcome', 'fusion-white-label-branding' ), 'fusion-white-label-branding-admin' );
			self::admin_tab( esc_html__( 'Support / FAQ', 'fusion-white-label-branding' ), 'fusion-white-label-branding-support' );
			self::admin_tab( esc_html__( 'Settings', 'fusion-white-label-branding' ), 'fusion-white-label-branding-settings' );
			self::admin_tab( esc_html__( 'Import / Export', 'fusion-white-label-branding' ), 'fusion-white-label-branding-import-export' );
			?>
		</h2>
		<?php
	}
